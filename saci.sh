...
function install() {
    ...
    arch-chroot /mnt pacman -S --noconfirm --needed --noprogressbar \
        cosmic                      `# Cosmic DE` \
        power-profiles-daemon       `# CPU power profile support via DE settings UI` \
    ...
    # xxx-aur-package: example for installing AUR package
    # exec_as_user "paru -S --noconfirm --needed --noprogressbar xxx-aur-package | tee -a $LOG_FILE"
    ...
    arch-chroot -u $USER_NAME /mnt git clone --recursive https://github.com/rstrube/saci.git /home/${USER_NAME}/saci
...
function confirm_install() {
    ...
    echo -e "${LBLUE}Sagi (Simple Arch Cosmic Installer)${NC}"
    echo ""
